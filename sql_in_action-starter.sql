...
-- no it seems like a cheap trash neighbourhood, thats there as an option for those who are on a tight budget
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 9 >>>>>>>>>>>>>>>>>>>>>>>
...
-- what makes a successful AirBnB is the right location, good pricing and consitant number of reviews each month.
-- <<<<<<<<<<<<<<<<<<<<< ** BONUS ** >>>>>>>>>>>>>>>>>>>>>>>
...
select id,(price/165.10)*100-100 as percent_above_or_below
from final_airbnb;
